...
timeout()
# базовая авторизация
response = requests.get(get_url('/api/authors/'), auth=('victor', 'biped090989'))
assert response.status_code == 200

timeout()
# авторизация по токену
TOKEN = requests.post(get_url('/api-token-auth/'), data={'username': 'victor', 'password': 'biped090989'}).json().get(
    'token')
...
# авторизация по jwt
# Получаем токен
response = requests.post(get_url('/api/token/'), data={'username': 'victor', 'password': 'biped090989'})
